...
{
    //This script will add new cubes and spheres randomly on the map
    //------VARIABLES-----//
    ...
    //To get a cleaner workspace, parent all soliders to these empty gameobjects
    public Transform enemyParent;
    public Transform friendlyParent;

    //Store all soliders in these lists
    List<Soldier> enemySoldiers = new List<Soldier>();
    List<Soldier> friendlySoldiers = new List<Soldier>();

    //Save the closest enemies to the easier change back its material
    List<Soldier> closestEnemies = new List<Soldier>();


    int currentNumSoldiers;
    bool canReset; 
    ...
    public void OnGUI()
    {
        ...
        if (GUILayout.Button("50 little guys"))
        {
            ...
            Start();
            numberOfSoldiers = currentNumSoldiers; 
        }

        if (GUILayout.Button("200 little guys"))
        {
            numberOfSoldiers = 200;
